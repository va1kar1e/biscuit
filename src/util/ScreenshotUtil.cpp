...
bool ScreenshotUtil::saveFramebufferAsBmp(const char* filename, const uint8_t* framebuffer, int width, int height) {
  ...
  // A root-level path such as "/sleep.bmp" has no parent directory to
  // create. substr(0, 0) is an empty string, and mkdir("") fails even though
  // the SD root already exists.
