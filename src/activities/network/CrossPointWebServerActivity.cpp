    ...
    if (!isApMode && webServer && webServer->isRunning()) {
      if (millis() - lastWifiCheck > 2000) {  // Check every 2 seconds
        ...
        if (wifiStatus != WL_CONNECTED) {
          ...
          // WiFi auto-reconnect handles brief dropouts. Only leave the screen
          // after a sustained outage, and go home cleanly instead of freezing
          // forever in SHUTTING_DOWN.
          if (millis() - firstDisconnectAt > WIFI_ABANDON_MS) {
            LOG_DBG("WEBACT", "WiFi unavailable for >%lu s; leaving file transfer", WIFI_ABANDON_MS / 1000UL);
            state = WebServerActivityState::SHUTTING_DOWN;
            onGoHome();
            return;
          }
