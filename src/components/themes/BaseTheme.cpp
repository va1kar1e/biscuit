...
void BaseTheme::drawStatusBar(GfxRenderer& renderer, const float bookProgress, const int currentPage,
  ...
  // Draw Title
  if (!title.empty()) {
    // Reader body text can use PK Nakhon Sawan, but the status-bar title used
    // SMALL_FONT_ID (Noto Sans 8), which has no Thai glyphs and displayed U+FFFD
    // replacement diamonds. Use the smallest bundled Thai font for book and
    // chapter titles when the Thai reader family is active.
    const int titleFontId = SETTINGS.fontFamily == CrossPointSettings::PKNAKHONSAWAN
                                ? PKNAKHONSAWAN_12_FONT_ID
                                : SMALL_FONT_ID;
    textY -= textYOffset;
