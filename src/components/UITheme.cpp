...
// PK Nakhon Sawan 12 has a 36px line advance (Thai marks extend above/below
// the Latin baseline), so its status-bar lane must reserve the same height.
constexpr int THAI_STATUS_BAR_TEXT_HEIGHT = 36;
}  // namespace
...
int UITheme::getStatusBarHeight() {
  ...
  const int textLaneHeight =
      SETTINGS.statusBarTitle != CrossPointSettings::STATUS_BAR_TITLE::HIDE_TITLE &&
              SETTINGS.fontFamily == CrossPointSettings::PKNAKHONSAWAN
          ? std::max(metrics.statusBarVerticalMargin, THAI_STATUS_BAR_TEXT_HEIGHT)
          : metrics.statusBarVerticalMargin;
  return (showStatusBar ? textLaneHeight : 0) +
